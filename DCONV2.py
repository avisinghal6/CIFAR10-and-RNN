...
def main():
    ...
    max_step = 3500 # the maximum iterations. After max_step iterations, the training will stop no matter what
    ...
    Z_conv1=conv2d(batch_xs, W_conv1) + b_conv1;
    h_conv1 = tf.nn.relu(Z_conv1);
    ...
    w1=tf.norm(H_POOL2,ord='euclidean',axis=[1,2])
    layer2_index=np.argpartition(tf.squeeze(w1),-9)[-9:]
    # layer2_index=np.argmax(,axis=1)
    ...
    # layer1_index=np.argmax(tf.norm(H_POOL1,ord='euclidean',axis=[1,2]),axis=1)
    w2=tf.norm(H_POOL1,ord='euclidean',axis=[1,2])
    layer1_index=np.argpartition(tf.squeeze(w2),-1)[-1:]
    ...
    for j in range(1):
        ...
        plt.imshow(batch_xs[0,:,:,0], cmap='gray')
        ax.set_xticks([])
        ax.set_yticks([])
        ix += 1
        ax = plt.subplot(1,3, ix)
