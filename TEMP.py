...
# l2=tf.norm(h_pool2,ord='euclidean',axis=[1,2])
# layer2_index=tf.math.argmax(tf.norm(h_pool2,ord='euclidean',axis=[1,2]),axis=1)
# idx2=28
...
# layer1_index=tf.argmax(tf.norm(h_pool2,ord='euclidean',axis=[1,2]))
# idx1=28
# LAYER1=tf.zeros(h_pool1.shape)
# LAYER1[:,:,:idx1]=h_pool1[:,:,:,idx1]
Unpooling1 = tfa.layers.MaxUnpooling2D()(h_pool1, max_index1)
Unrelu1= tf.nn.relu(Unpooling1);
Dconv1=conv2d(Unrelu1, tf.transpose(W_conv1,perm=[1,0,3,2]));
...
# --------------------------------------------------
ix=1
H_POOL2=h_pool2.eval(feed_dict={tf_data:batch_xs, tf_labels:batch_ys, keep_prob: 1})
MAX_INDEX2=max_index2.eval(feed_dict={tf_data:batch_xs, tf_labels:batch_ys, keep_prob: 1})
W_CONV2=W_conv2.eval()

H_POOL1=h_pool1.eval(feed_dict={tf_data:batch_xs, tf_labels:batch_ys, keep_prob: 1})
MAX_INDEX1=max_index2.eval(feed_dict={tf_data:batch_xs, tf_labels:batch_ys, keep_prob: 1})
W_CONV1=W_conv1.eval()
sess.close()
...
for i in range(1):
    ...
    H_POOL2=H_POOL2.numpy()
    layer2_index=np.argmax(np.norm(H_POOL2,ord='euclidean',axis=[1,2]),axis=1)
    LAYER2=np.zeros(H_POOL2.shape)
    LAYER2[:,:,:,layer2_index]=H_POOL2[:,:,:,layer2_index]
    LAYER2=tf.convert_to_tensor(LAYER2)

    
    unpooling2 = tfa.layers.MaxUnpooling2D()(LAYER2, MAX_INDEX2)
    unrelu2= tf.nn.relu(unpooling2);
    dconv2=conv2d(unrelu2, tf.transpose(W_CONV2,perm=[1,0,3,2]));
    ...
    H_POOL1=H_POOL1.numpy()
    layer1_index=np.argmax(np.norm(H_POOL1,ord='euclidean',axis=[1,2]),axis=1)
    LAYER1=np.zeros(H_POOL1.shape)
    LAYER1[:,:,:,layer1_index]=H_POOL1[:,:,:,layer1_index]
    LAYER1=tf.convert_to_tensor(LAYER1)
    Unpooling1 = tfa.layers.MaxUnpooling2D()(LAYER1, MAX_INDEX1)
    Unrelu1= tf.nn.relu(Unpooling1);
    Dconv1=conv2d(Unrelu1, tf.transpose(W_conv1,perm=[1,0,3,2]));
